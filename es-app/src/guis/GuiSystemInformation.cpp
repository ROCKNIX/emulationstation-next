	...
	// --- ROCKNIX MEMORY STATUS ---
	{
		...
		if (pipe) {
			...
			if (!doc.HasParseError() && doc.IsObject()) {
				...
				// Overcommit
				std::string ocText = "Heuristic (0)";
				if (doc.HasMember("overcommit_memory") && doc["overcommit_memory"].IsInt()) {
					int oc = doc["overcommit_memory"].GetInt();
					if (oc == 1) ocText = "Always (1)";
					else if (oc == 2) ocText = "Strict (2)";
				}
				addWithLabel(_("VM OVERCOMMIT"), std::make_shared<TextComponent>(window, ocText, font, color));
			}
		}
	}
}
